...
| 관리 | 관리 토큰으로 제목, 설명, 컬럼 라벨, 공개 여부, 엑셀 데이터 교체, 삭제 |
| 운영 | 신고 상태 관리, 감사 로그, 관리/삭제 요청 제한 |
...
4. 주소 변환 진행률을 확인합니다.
5. 발급된 공개 지도 링크를 공유하고, 관리 토큰은 따로 보관합니다.
...
- `/manage/[slug]` — 업로드한 지도 정보 수정, 엑셀 데이터 교체, 비공개 전환 및 삭제
...
- 비동기 업로드 작업 API, 모바일 필터 패널, 관리 페이지 데이터 교체 안내
