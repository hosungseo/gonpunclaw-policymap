...
공개 지도에서 `행정구역 경계 표시`를 켜면 `public/data/sido-boundaries.geojson` 또는
`public/data/sigg-boundaries.geojson` 정적 파일을 읽어 광역시도/시군구 경계를 표시합니다.
이 파일은 VWorld Data API 2.0 `LT_C_ADSIDO_INFO`, `LT_C_ADSIGG_INFO`에서 받은 경계를
지도 표시용으로 단순화한 데이터입니다.
