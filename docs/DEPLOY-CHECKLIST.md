...
## 3. 배포 환경 변수
실제 입력 기준은 `docs/PRODUCTION-ENV.md`를 먼저 보고, 아래 체크리스트로 누락만 확인합니다.
...
## 7. 실배포 후 메모
- 2026-04-24 기준 Vercel production 배포 완료
- 기본 env와 Kakao geocoder env까지 반영 완료
- 실서버 QA에서 `/`, `/upload`, `/staff/audit` 200 응답 확인
- 실업로드 QA에서 `.xlsx` 샘플 업로드 성공, geocoding 2건 성공, 공개 지도/관리 페이지 확인 완료
- `.csv` 샘플은 `BAD_HEADER`로 실패해 현재 운영 가이드는 `.xlsx` 우선이 안전함
